...
class Grass(pygame.sprite.Sprite):
    def __init__(self, screen_x, screen_y, screen, x=None, y=None):
        ...
        self.color = tuple(min(255, max(0, c + random.randint(-20, 20))) for c in (100, 180, 90))
        ...
        #wind
        self.wind_time = random.uniform(0, 2 * math.pi)  # phase offset
        self.wind_amplitude = random.uniform(0.5, 2.0)
        self.wind_speed = random.uniform(0.02, 0.05)
        self.sway_offset = 0

    def update(self, grass_group, season, tree_group, house_group):
        self.sway_offset += self.wind_speed
